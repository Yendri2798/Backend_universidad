...
DROP TABLE Consejero CASCADE CONSTRAINTS;
...
DROP TABLE Prematricula CASCADE CONSTRAINTS;
*/
...
create or replace PROCEDURE insertaCurso(codCur in curso.codigo_Curso%TYPE, nom in curso.nombre%TYPE,
                                         cred in curso.creditos%TYPE, hora in curso.horas_semanales%TYPE,
                                         carrera_cod in curso.carrera_codigo%TYPE)
as
begin
...
create or replace PROCEDURE insertaProfesor(ced in profesor.cedula_Profesor%TYPE, nom in profesor.nombre%TYPE,
                                            apel in profesor.primer_apellido%TYPE, tel in profesor.telefono%TYPE,
                                            correo in profesor.email%TYPE)
as
begin
...
create or replace PROCEDURE insertaAlumno(ced in alumno.cedula_alumno%TYPE, nom in alumno.nombre%TYPE,
                                          apel in alumno.primer_apellido%TYPE, tel in alumno.telefono%TYPE,
                                          email in alumno.email%TYPE, fecha in alumno.fecha_nacimiento%TYPE,
...
create or replace PROCEDURE insertaCiclo(an in ciclo.annio%TYPE, nume in ciclo.numero%TYPE,
                                         fecha_Ini in ciclo.fecha_Incio%TYPE,
...
create or replace PROCEDURE modificaCiclo(anni in ciclo.annio%TYPE, numer in ciclo.numero%TYPE,
                                          fechaInicio in ciclo.fecha_Incio%TYPE,
...
create or replace PROCEDURE insertaGrupo(num1 in grupo.numero_grupo%TYPE, hora in grupo.horario%TYPE,
                                         campos in grupo.campos_Restantes%TYPE,
                                         capacidad in grupo.capacidad_Maxima%TYPE, ciclo1 in grupo.ciclo_annio%TYPE,
                                         curs in grupo.curso_codigo%TYPE, prof in grupo.profesor_cedula%TYPE,
                                         num_ciclo in grupo.numero_ciclo%TYPE)
as
begin
