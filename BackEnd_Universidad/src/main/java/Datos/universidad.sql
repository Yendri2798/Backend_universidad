...
/*Actualización al 13-04-2022*/

/*NOTA*/

create or replace PROCEDURE insertaNota(idCurso in nota_alumno.id_curso%TYPE, note in nota_alumno.nota%TYPE,
ced_alum in nota_alumno.alumno_cedula%TYPE)
as
begin
insert into nota_alumno values(idCurso, note, ced_alum);
end;
/

create or replace PROCEDURE modificaNota(idCurso in nota_alumno.id_curso%TYPE, note in nota_alumno.nota%TYPE,
ced_alum in nota_alumno.alumno_cedula%TYPE)
as
begin
UPDATE nota_alumno SET nota = note where alumno_cedula = ced_alum and id_curso = idCurso;
end;
/

CREATE OR REPLACE FUNCTION buscarNota(idbuscar IN nota_alumno.alumno_cedula%TYPE)
RETURN Types.ref_cursor 
AS 
...
BEGIN 
  OPEN nota_cursor FOR 
       ...
       FROM nota_alumno WHERE alumno_cedula = idbuscar; 
RETURN nota_cursor; 
END;
/
...
create or replace PROCEDURE insertaConsejero(alumCed in consejero.alumno_cedula%TYPE, 
profeCed in consejero.profesor_cedula%TYPE)
as
begin
insert into Consejero values(alumCed, profeCed);
end;
/

CREATE OR REPLACE FUNCTION buscarConsejeroProfe(idbuscar IN consejero.profesor_cedula%TYPE)
RETURN Types.ref_cursor 
AS 
...
BEGIN 
  OPEN consejero_cursor FOR 
       ...
       FROM Consejero WHERE profesor_cedula = idbuscar; 
RETURN consejero_cursor; 
END;
/

CREATE OR REPLACE FUNCTION buscarConsejeroAlumno(idbuscar IN consejero.alumno_Cedula%TYPE)
RETURN Types.ref_cursor 
AS 
        consejero_cursor types.ref_cursor; 
BEGIN 
...
create or replace PROCEDURE insertaPrematricula( 
...
as
begin
insert into Prematricula(estado, alumno_cedula, grupo_num,grupo_Ciclo) values(estado, cedAlum, numGrupo, grupociclo);
end;
/

create or replace PROCEDURE modificaPrematricula(idP in prematricula.idpre%TYPE, 
est in prematricula.estado%TYPE)
as
begin
UPDATE Prematricula SET estado = est where idpre = idP;
end;
/

create or replace PROCEDURE eliminaPrematricula(idP in prematricula.idpre%TYPE)
as
begin
DELETE FROM Prematricula where idpre = idP;
end;
/

CREATE OR REPLACE FUNCTION buscarPrematricula(idbuscar IN prematricula.alumno_cedula%TYPE)
RETURN Types.ref_cursor 
AS 
...
BEGIN 
  OPEN prematricula_cursor FOR 
       ...
       FROM Prematricula WHERE alumno_cedula = idbuscar; 
...
create or replace PROCEDURE insertaAnalisis(cedAlum in analisis.consejero_Alumno%TYPE, 
cedProf in analisis.consejero_profesor%TYPE, id_Pret in analisis.prematricula_id%TYPE)
as
begin
insert into Analisis values(cedAlum, cedProf, id_Pret);
end;
/

create or replace PROCEDURE eliminaAnalisis(idPret in analisis.prematricula_id%TYPE)
as
begin
DELETE FROM Analisis where prematricula_id = idPret;
end;
/

CREATE OR REPLACE FUNCTION buscarAnalisis(idbuscar IN analisis.consejero_profesor%TYPE)
RETURN Types.ref_cursor 
AS 
...
BEGIN 
  OPEN analisis_cursor FOR 
       ...
       FROM Analisis WHERE consejero_profesor = idbuscar; 
...
create or replace PROCEDURE insertaMatricula(idPremat in matricula.prematric_id%TYPE, 
cedAlum in matricula.alumno_cedula%TYPE)
as
begin
insert into Matricula values(idPremat, cedAlum);
end;
/
